...
std::string ReplaceNullCharacters(const std::string& inputString) {
...
/// @todo testen
size_t Read16ByteValuesFromBuffer(const char* buffer, size_t startIndex)
{
    try {
...
/// @todo testen
size_t ReadByteValuesFromBuffer(const char* buffer, size_t startIndex, size_t numBytesToRead)
{
    try {
    size_t bufferSize = sizeof(buffer);
    if (startIndex >= bufferSize)
